            ...
            try:
                for root_dir, dirs, files in os.walk(base):
                    for name in dirs + files:
                        ...
                        # 风险修复：如果只是文件内容包含 openclaw (如 .claude/config.json)
                        # 直接整文件删除会导致其他如 OpenAI 的环境配置和插件被错杀，因此仅记录警告，不加入直删列表。
                        if not is_claw and base == os.path.join(UP, ".claude") and self._has_claw_content(full):
                            self._log(f"  ⚠ 发现内容含 OpenClaw 记录，建议手动检查：{full}", "warn")
